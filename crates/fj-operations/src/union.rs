...
use fj_kernel::{
    algorithms::{intersection, Tolerance},
    shape::Shape,
};
use fj_math::Aabb;
        ...
        for face_a in a.faces() {
            for face_b in b.faces() {
                ...
                let intersection_curve =
                    match intersection::surface_surface(&surface_a, &surface_b)
                    {
                        Some(curve) => curve,
                        None => {
                            // If we don't get an intersection curve, we treat
                            // the surfaces as not intersecting, and therefore
                            // not relevant.
                            //
                            // This is not fully correct, as there are edge
                            // cases that this doesn't handle, like coincident
                            // surfaces.
                            continue;
                        }
                    };
